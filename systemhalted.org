...
SDKMAN (or the distro's =update-alternatives=) currently considers default ---
no =lsp-java-configuration-runtimes= needed, and switching JDKs is a matter
of =sdk use java <version>= (or the distro equivalent) followed by an Emacs
restart.
