...
Lock files (=.#filename= symlinks Emacs drops next to a buffer on first edit) are
disabled outright: this is a single-user setup where the concurrent-edit warning
effectively never fires, and the symlinks confuse external watchers (bundlers,
file-system watchers, language servers, Git GUIs).
...
Long-running Emacs sessions can become memory-bound if redisplay is forced to
combine expensive features in large or visually complex buffers. Line numbers are
therefore enabled only for ordinary local programming buffers, and =so-long=
protects buffers with pathological long lines. If the UI starts to degrade,
=C-c != runs =systemhalted/emergency-ui-reset= to disable expensive UI/runtime
features and force a garbage collection. If Emacs is unresponsive on macOS,
sample the process from a terminal with:
