...
(use-package lsp-mode
  ...
  ;; Default is 10s. JDT LS's textDocument/codeAction on a freshly-imported
  ;; project — especially one with parse errors — routinely needs longer
  ;; than that, which surfaces as `C-c l a a' "timing out" (the request is
  ;; cancelled by post-command-hook before the server replies).
